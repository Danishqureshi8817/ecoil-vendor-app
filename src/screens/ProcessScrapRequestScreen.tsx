...
import {
  ...
  scrapCollectionId,
  scrapFirmName,
  type ScrapCategorySubmitItem,
  type ScrapPicturePayload,
  type ScrapRequestRow,
} from '@/api/scrapApi';
import {Colors} from '@/constants/colors';
import {Fonts} from '@/constants/fonts';
import {useCompleteScrapRequest} from '@/hooks/vendor/use-scrap-requests';
...
type Props = NativeStackScreenProps<
  RootStackParamList,
  typeof StackNav.ProcessScrapRequest
>;
...
function emptyCategoryForm(scrap_category_id: string): ScrapCategorySubmitItem {
  return {
    scrap_category_id,
    weight: '',
    scrap_picture1: null,
...
function fromDocument(
  ...
  return toPicturePayload(
    uri,
    res.name ?? `scrap_picture${slot}.jpg`,
    res.type ?? 'image/jpeg',
  );
}
...
async function pickFromGallery(slot: number): Promise<ScrapPicturePayload | null> {
  ...
  if (imagePicker?.launchImageLibrary) {
    ...
    if (result.errorCode === 'permission') {
      showPermissionSettingsAlert(
        ...
        'Please allow photo access in Settings to choose scrap pictures.',
      );
      return null;
    }
    if (result.errorCode) {
    ...
    return toPicturePayload(
      asset.uri,
      asset.fileName ?? `scrap_picture${slot}.${ext}`,
  ...
  const [localCopy] = await keepLocalCopy({
    files: [
      {
        uri: picked.uri,
        fileName: picked.name ?? `scrap_picture${slot}.jpg`,
      },
    ],
...
async function pickFromCamera(slot: number): Promise<ScrapPicturePayload | null> {
  const imagePicker = getImagePicker();
  ...
  if (result.errorCode === 'permission') {
    showPermissionSettingsAlert(
      ...
      'Please allow camera access in Settings to take scrap pictures.',
    );
    return null;
  }
  if (result.errorCode) {
...
export default function ProcessScrapRequestScreen({route}: Props) {
  const request = route.params.request as ScrapRequestRow;
  const user = useAuthStore(s => s.user);
  const {toastSuccess} = useToastMessage();
  const completeMutation = useCompleteScrapRequest();

  const categories = useMemo(() => parseScrapCategories(request), [request]);
  const branchName = useMemo(() => scrapBranchDisplay(request), [request]);

  const [forms, setForms] = useState<ScrapCategorySubmitItem[]>(() =>
    categories.map(c => emptyCategoryForm(c.scrap_category_id)),
  );
  ...
  function updateCurrent(patch: Partial<ScrapCategorySubmitItem>) {
    setForms(prev =>
  ...
  function validateCurrentStep(): boolean {
    ...
    if (!hasPicture) {
      setError('Add at least one scrap picture');
      return false;
    }
    setError('');
    return true;
  ...
  async function pickImage(from: 'camera' | 'gallery') {
    ...
    const slotNumber = Number(slot.replace('scrap_picture', ''));
    setPickerSlot(null);
    setError('');
  ...
  async function onSubmit() {
    const collectionId = scrapCollectionId(request);
    if (!collectionId) {
      setError('Missing scrap_collection_id');
      return;
    }
    ...
    try {
      ...
      toastSuccess('Scrap request submitted');
      resetToDrawerScreen(StackNav.ScrapRequests);
    } catch (err) {
      setError(getApiErrorMessage(err, 'Could not submit scrap request'));
    }
  }
  ...
  if (categories.length === 0) {
    return (
      <Container fullScreen statusBarStyle="light-content">
        <AppBar title="Process Scrap Request" leading="back" />
        <View style={styles.emptyWrap}>
          ...
          <CustomText
            ...
            style={styles.muted}>
            This request has no scrap categories to process.
          </CustomText>
        </View>
      </Container>
    );
  ...
  return (
    <Container fullScreen statusBarStyle="light-content">
      <AppBar
        title={isPreviewStep ? 'Preview & Submit' : 'Process Scrap Request'}
        leading="back"
          ...
          {isPreviewStep ? (
            <View style={styles.previewList}>
              {forms.map((form, index) => (
                <View
                  key={form.scrap_category_id + index}
                  style={styles.previewCard}>
                  <CustomText
                    variant="h6"
                    fontFamily={Fonts.montserrat.bold}
                    style={styles.previewTitle}>
                    {categories[index]?.name ?? form.scrap_category_id}
                  </CustomText>
                  <CustomText
                    variant="h7"
                    fontFamily={Fonts.montserrat.medium}
            ...
            <View style={styles.formCard}>
              ...
              <View style={styles.readOnlyField}>
                <CustomText
                  ...
                  {currentCategory?.name ?? currentForm?.scrap_category_id}
                </CustomText>
              </View>
